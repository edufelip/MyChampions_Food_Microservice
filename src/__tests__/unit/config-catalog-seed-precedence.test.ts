describe('config catalog seed precedence', () => {
  ...
  it('uses JSON defaults when CATALOG_SYNC_SEED_QUERIES is not provided', async () => {
    delete process.env.CATALOG_SYNC_SEED_QUERIES;
    jest.resetModules();
  ...
  it('uses env override when CATALOG_SYNC_SEED_QUERIES has at least 50 entries', async () => {
  ...
  it('throws when CATALOG_SYNC_SEED_QUERIES has less than 50 entries', async () => {
    process.env.CATALOG_SYNC_SEED_QUERIES = 'rice, beans, oats';
    jest.resetModules();

    await expect(import('../../config')).rejects.toThrow('CATALOG_SYNC_SEED_QUERIES must include at least 50 entries');
  });
});
